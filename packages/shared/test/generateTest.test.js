...
import {
  ...
  generateTest,
  shuffle,
} from "../src/features/test-session/generate.ts";
import { stateKeys } from "../src/app/uklad/catalog.ts";
...
function createMockDraftDb(questionsData, usedTestQuestions = {}) {
  ...
  const categoryGroups = [{ title: "Themes", items: categoryItems }];

  return {
    [stateKeys.questionsCategories]: categoryGroups,
    [stateKeys.questionsItems]: questions,
    [stateKeys.testSessionUsedQuestions]: usedTestQuestions,
    [stateKeys.testSessionQuestions]: [],
    [stateKeys.testSessionAnswers]: {},
    // Keep the assertions readable while exercising the Uklad-shaped
    // state roots above.
    get categories() {
      return this[stateKeys.questionsCategories];
    },
    get questions() {
      return this[stateKeys.questionsItems];
    },
    get usedTestQuestions() {
      return this[stateKeys.testSessionUsedQuestions];
    },
    get testQuestions() {
      return this[stateKeys.testSessionQuestions];
    },
    get testAnswers() {
      return this[stateKeys.testSessionAnswers];
    },
  };
}
...
describe("calculateAllocation", () => {
  it("allocates proportionally with simple rounding", () => {
    ...
    const result = calculateAllocation(topicSizes, 15);
  ...
  it("allocates all to largest topic when others are too small", () => {
    ...
    const result = calculateAllocation(topicSizes, 5);
...
describe("generateTest", () => {
  ...
  it("handles multiple generations correctly", () => {
    ...
    const draftDb2 = createMockDraftDb(
      mockQuestions,
      draftDb1.usedTestQuestions,
    );
  ...
  describe("Real-world German citizenship test scenarios", () => {
    ...
    const germanTestQuestions = {
      "Leben in der Demokratie": Array.from({ length: 50 }, (_, i) => ({
        id: i + 1,
      })),
      "Geschichte und Verantwortung": Array.from({ length: 50 }, (_, i) => ({
        id: i + 51,
      })),
      "Mensch und Gesellschaft": Array.from({ length: 50 }, (_, i) => ({
        id: i + 101,
      })),
      "Baden-Württemberg": Array.from({ length: 10 }, (_, i) => ({
        id: i + 151,
      })), // State-specific
    };
    ...
    it("handles repeated test taking with auto-reset behavior", () => {
      ...
      const totalUsedAfter5Tests = Object.values(currentUsed).reduce(
        (sum, topicUsed) => sum + Object.keys(topicUsed).length,
        0,
      );
  ...
  describe("updatedUsed tracking correctness", () => {
    it("preserves existing used questions and adds new ones or resets when exhausted", () => {
      ...
      console.log(
        "Object.keys(draftDb.usedTestQuestions.Topic)",
        Object.keys(draftDb.usedTestQuestions.Topic),
      );
  ...
  describe("Randomization and distribution", () => {
    ...
    it("maintains proportional distribution across topics", () => {
      ...
      const bigTopicCount = draftDb.testQuestions.filter(
        (q) => q.id <= 60,
      ).length;
      const mediumTopicCount = draftDb.testQuestions.filter(
        (q) => q.id > 60 && q.id <= 90,
      ).length;
      const smallTopicCount = draftDb.testQuestions.filter(
        (q) => q.id > 90,
      ).length;
  ...
  describe("German citizenship test realistic scenarios", () => {
    ...
    it("simulates realistic user progression through multiple test sessions", () => {
      ...
      testSessions.forEach((session) => {
        ...
        const categoriesWithQuestions = Object.values(
          session.categories,
    ...
    it("handles state-specific question requirements", () => {
      const stateQuestions = {
        "National Topic 1": Array.from({ length: 100 }, (_, i) => ({
          id: i + 1,
        })),
        "National Topic 2": Array.from({ length: 100 }, (_, i) => ({
          id: i + 101,
        })),
        "Baden-Württemberg": Array.from({ length: 10 }, (_, i) => ({
          id: i + 201,
